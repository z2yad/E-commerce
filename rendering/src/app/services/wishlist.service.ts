...
export class WishlistService {
  ...
  private readonly normalizeId = (id: string | number) => String(id);
  ...
  removeFromWishlist(productId: string | number): boolean {
    const currentLength = this.wishlistItems().length;
    this.wishlistItems.update(items =>
      items.filter(item => this.normalizeId(item.id) !== this.normalizeId(productId))
    );
  ...
  isInWishlist(productId: string | number): boolean {
    return this.wishlistItems().some(
      (product) => this.normalizeId(product.id) === this.normalizeId(productId)
    );
  }
